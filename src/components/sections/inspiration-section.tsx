...
interface VideoPreviewData {
  ...
  imageAlt: string;
  aiHint: string;
  videoSrc: string; // Added video source URL
}
        ...
        <div className="flex flex-col lg:flex-row items-center gap-8 md:gap-12 lg:gap-16">
          ...
          <div className="lg:w-1/3 text-center lg:text-left">
            ...
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-foreground mb-4">
              Discover Creations from <span className="text-primary">LumoraAI Studios</span>
            </h2>
            <p className="text-muted-foreground mb-8 text-lg">
              See what our users are making with LumoraAI Studios. From engaging educational content
          ...
          <div className="lg:w-2/3 w-full">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {videoPreviews.map((video) => (
                <Card
                  key={video.id}
                  className="rounded-xl overflow-hidden shadow-lg hover:shadow-2xl hover:scale-105 transition-all duration-300 ease-in-out group"
                >
                  <div className="aspect-video relative" onClick={() => openVideo(video.id)}>
                    <Image
                      src={video.imageSrc}
                      alt={video.imageAlt}
                      fill
        ...
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
          <div className="relative w-full max-w-4xl">
            ...
            <div className="rounded-lg overflow-hidden aspect-video w-full bg-black">
              <video 
                src={videoPreviews.find(v => v.id === activeVideo)?.videoSrc} 
                controls 
                autoPlay 
